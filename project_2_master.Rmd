...
```{r}
tibble(coef_final[coef_final != 0])
...
```
Our training model gave us a 91.6% correct classification rate with our test data set, suggesting a very strong model that is consistent and reproducible. 
```{r}
...
```
Finally, our Precision Recall Area Under the Curve curve shows that 93.37% of the data is under the curve, showing a very effective model in predicting the low class, or the class that has low grades. This is important as it shows the group that is need of an academic intervention correctly more times or not. So with this model, we are able 
...
So our LASSO regression model performs at a very high rate in correctly identifying our minority low groups. Our PR-AUC is the best indication of the effectiveness of this model as we have show and stated above with a very high 93.37% correct rate of predicting our low groups. The one downside to this model is how many predictors it requires for it to be accurate, but with a complete data set like this it works exceptionally well. 
