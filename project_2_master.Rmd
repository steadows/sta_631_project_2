...
only 144 were correctly identified as low, but 103 were misclassified as high,
...
Recall (True negative (Low) + false negative (Low)) = 51 / 51+116 = 30.54%
Precision (Our predicted negatives or Low's) = 51 / 51+6 = 89.47%
...
After completing three separate modeling approaches (LDA, LASSO Logistic Regression, and SVM Classifier) we observed clear differences in predictive performance, with our main focus being on the identification of low students.
...
LASSO Logistic Regression appeared to have the best overall predictive ability, with 91.6% accuracy and an AUC of 0.9337. While recall for the low-success class was low (30.54%), an good precision score (89.47%), and had an excellent overall accuracy (91.6%) the model still misclassified low-success students as high at an alarming rate. Which is bound to happen as no model ever has been 100% completely perfect without multicollinearity and overfitting the data.

SVM Classifier achieved a high recall (=89.5%) for low-success students, and had a good precision score (=40.8%) and overall accuracy (=81.5%). The PR-AUC (0.634) showed strong low ranking performance, and the precision–recall curve shows the model’s capacity to detect at-risk low students consistently.

Overall, if our goal is to identify those students who are in desperate need of help (Low students), we will want to pick the model that best predicts those students who are low and are predicted low at a high rate, but more importantly having a low rate of students who are predicted high who are low. Right away, we can tell that it is between our LDA and our SVM Classifier models to use as our final model as LASSO had a lower overall and recall accuracy compared to the other two. 

So between our LDA and SVM Classifier models. Looking at both, they each have very different recall percentages indicating that one correctly predicts a low student at a better rate. Our biggest concern is looking at our recall percentage as we do not want to identify low students as high as they will not receive the needed help. With that, our best model would be our SVM Classifier model as it blows LASSO and LDA out with an extremely high value of 89.5% as LDA only has a recall percentage of 58% which is significantly lower then our SVM Classifier model.

In conclusion, our final chosen model is the SVM Classifier model, as it maintains a better recall value than the LASSO model and LDA model. In addition, this model will give us the best chance at identifying low students correctly, provides very few false negatives (predicted high but is low), and still provides a serviceable precision score. 
